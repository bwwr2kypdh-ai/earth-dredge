...
# Injection CSS (Gestion propre de l'impression des onglets SANS forcer l'opacité des cartes)
st.markdown(""" 
<style> 
@media print { 
...
# --- SECURITE ANTI-CRASH (Initialisation des variables) ---
current_bilan = current_deblai = current_remblai = current_m_rem = current_m_deb = current_lost_area = 0.0
...
if col_opt1.button("🚀 CALCULER", type="primary", use_container_width=True):
...
target_annual_teu = st.sidebar.number_input("Trafic Annuel (TEU)", value=100000) 
dwell_time = st.sidebar.number_input("Temps de Sejour (Jours)", value=7) 
...
admin_sqm = st.sidebar.number_input("Superficie Batiments (m2)", value=1500) 
lane_cap = st.sidebar.number_input("Capacite / Voie Gate", value=25000, min_value=1) # Securité ZeroDivisionError
...
if st.session_state['raw_df'] is not None: 
    ...
    # ALGORITHMES DE RECHERCHE DE FORMES (Rectangle, Triangle Rectangle, Losange)
    ...
    def get_max_inscribed_parallelogram_robust(poly, is_auto, man_angle):
        ...
        centroid = (poly.centroid.x, poly.centroid.y)
        angles_to_test = range(0, 180, 10) if is_auto else [man_angle]
        shear_angles = range(-45, 46, 15) # Test de cisaillement inverse (skew)
    ...
    with col_pdf2: 
        st.caption("💡 Imprimez au format Paysage (Landscape) pour un meilleur rendu.")
    ...
    with tab_civil: 
        ...
        st.caption(f"Ligne Verte = Raccordement parfait. Ligne Magenta pointillée = Emprise utile nette. Bloc Jaune = {current_shape_type} Optimisé.") 
